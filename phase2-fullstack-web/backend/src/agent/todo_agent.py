...
from agents import Agent, function_tool, RunContextWrapper, ModelSettings, OpenAIChatCompletionsModel
from sqlmodel import Session

from .config import AgentConfig, get_agent_config, get_api_client
...
def create_todo_agent(config: Optional[AgentConfig] = None, user_name: Optional[str] = None) -> Agent[AgentContext]:
    ...
    Uses Gemini or OpenAI based on available API keys.
    """
    if config is None:
        config = get_agent_config()

    # Create custom model using the configured API client (Gemini or OpenAI)
